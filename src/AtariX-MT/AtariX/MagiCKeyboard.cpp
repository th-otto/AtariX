...
unsigned char CMagiCKeyboard::GetModifierScanCode(UInt32 modifiers, bool *bAutoBreak)
{
	...
	// Änderungen zu bisher gesendetem Status durch XOR berechnen

	diffmod = modifiers ^ m_modifiers;
	if	(!diffmod)
		return(0);

	// Alle Bits auf Unterschiede prüfen
	...
	*bAutoBreak = (ret == 58);		// Atari-CapsLock
	...
	if	(modifiers & mask)
		...
		if	(!(*bAutoBreak))	// Für CapsLock wird der Break-Code automatisch generiert
			ret |= 0x80;
		m_modifiers &= ~mask;
		}
