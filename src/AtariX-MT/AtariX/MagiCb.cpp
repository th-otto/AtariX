...
#endif
static unsigned char *PPCVideoAddr;		// Beginn Bildschirmspeicher PPC

static unsigned char *PPCVideo2Addr;		// Beginn Bildschirmspeicher PPC (Hintergrundpuffer)
...
m68k_addr_type m68k_read_memory_8(m68k_addr_type address)
{
	...
	{
		return(*((UINT8*) (PPCVideo2Addr + (address - Adr68kVideo))));
	}
	else
	{
		const char *Name;
...
m68k_addr_type m68k_read_memory_16(m68k_addr_type address)
{
	...
	if	(address < Adr68kVideoEnd)
	{
		val = *((UINT16*) (PPCVideo2Addr + (address - Adr68kVideo)));
//		return(CFSwapInt16BigToHost(val));
...
m68k_addr_type m68k_read_memory_32(m68k_addr_type address)
{
	...
	if	(address < Adr68kVideoEnd)
	{
		val = *((UINT32*) (PPCVideo2Addr + (address - Adr68kVideo)));
//		return(CFSwapInt32BigToHost(val));
		return val;		// x86 has bgr instead of rgb
	}
	else
...
void m68k_write_memory_8(m68k_addr_type address, m68k_data_type value)
{
	...
	{
		address -= Adr68kVideo;
		*((UINT8*) (PPCVideoAddr + address)) = (UINT8) value;
		*((UINT8*) (PPCVideo2Addr + address)) = (UINT8) value;
	}
	else
	{
		const char *Name;
...
void m68k_write_memory_16(m68k_addr_type address, m68k_data_type value)
{
	...
	{
		address -= Adr68kVideo;
//		*((UINT16*) (PPCVideoAddr + address)) = (UINT16) CFSwapInt16HostToBig(value);
		*((UINT16*) (PPCVideoAddr + address)) = (UINT16) value;		// x86 has bgr instead of rgb
//		*((UINT16*) (PPCVideo2Addr + address)) = (UINT16) CFSwapInt16HostToBig(value);
		*((UINT16*) (PPCVideo2Addr + address)) = (UINT16) value;	// x86 has bgr instead of rgb
	}
	else
	{
		const char *Name;
...
#endif
		address -= Adr68kVideo;
		*((UINT32*) (PPCVideoAddr + address)) = value;		// x86 has brg instead of rgb
//		*((UINT32*) (PPCVideoAddr + address)) = CFSwapInt32HostToBig(value);
		*((UINT32*) (PPCVideo2Addr + address)) = value;		// x86 has brg instead of rgb
//		*((UINT32*) (PPCVideo2Addr + address)) = CFSwapInt32HostToBig(value);
	}
	else
	{
		const char *Name;
...
CMagiC::~CMagiC()
{
	...
	{
		// Task abschießen
		MPTerminateTask(m_EmuTaskID, noErr);
		// Auf Beendigung warten
		MPWaitOnQueue(m_EmuNotifQID, NULL, NULL, NULL, kDurationForever);
	}
	if	(m_EmuNotifQID)
		MPDeleteQueue(m_EmuNotifQID);
	if	(m_EventId)
		MPDeleteEvent(m_EventId);
	if	(m_InterruptEventsId)
		MPDeleteEvent(m_InterruptEventsId);
	if	(m_KbCriticalRegionId)
		MPDeleteCriticalRegion(m_KbCriticalRegionId);
	if	(m_AECriticalRegionId)
		MPDeleteCriticalRegion(m_AECriticalRegionId);
	if	(m_ScrCriticalRegionId)
		MPDeleteCriticalRegion(m_ScrCriticalRegionId);
	if	(m_RAM68k)
		free(m_RAM68k);
}
...
static void PixmapToBigEndian(PixMap *thePixMap)
{
	DebugInfo("Host-CPU ist little-endian. Rufe PixmapToBigEndian() auf.");

	thePixMap->baseAddr = (char *) CFSwapInt32HostToBig((UInt32) thePixMap->baseAddr);
	thePixMap->rowBytes = CFSwapInt16HostToBig(thePixMap->rowBytes);
	thePixMap->bounds.top = CFSwapInt16HostToBig(thePixMap->bounds.top);
	thePixMap->bounds.left = CFSwapInt16HostToBig(thePixMap->bounds.left);
	thePixMap->bounds.bottom = CFSwapInt16HostToBig(thePixMap->bounds.bottom);
	thePixMap->bounds.right = CFSwapInt16HostToBig(thePixMap->bounds.right);
...
int CMagiC::Init(CMagiCScreen *pMagiCScreen, CXCmd *pXCmd)
{
...
#endif
#ifdef WATCH_68K_PC
	DebugInfo("68k-PC wird auf Gültigkeitigkeit überprüft (Emulator läuft etwas langsamer)");
#else
	...
	m_RAM68k = (unsigned char *) malloc(Globals.s_Preferences.m_AtariMemSize);
	if	(!m_RAM68k)
	{
/*
...
*/
	...
	Adr68kVideoEnd = Adr68kVideo + Globals.s_PPCVideoBytes;
	...
	*((UINT16 *)(m_RAM68k+_nflops)) = CFSwapInt16HostToBig(0);		// keine Floppies
	...
	pAtari68kData->m_PixMap.bounds.right = m_pMagiCScreen->m_AtariScreenRect00.right;
	pAtari68kData->m_PixMap.bounds.bottom = m_pMagiCScreen->m_AtariScreenRect00.bottom;
	pAtari68kData->m_PixMap.baseAddr = (char *) Adr68kVideo;		// virtuelle 68k-Adresse
...
//	DumpAtariMem("AtariMemAfterInit.data");
	...
	*((UINT32 *)(m_RAM68k + _drvbits)) = CFSwapInt32HostToBig(0);		// noch keine Laufwerke
	FSMakeFSSpec (0, 0, "\pMAGIC_C", &CDriveSpec);
	NewAlias(&Globals.s_ProcDir, &CDriveSpec, &Globals.s_Preferences.m_drvAlias['C'-'A']);
	m_MacXFS.SetXFSDrive(
					'C'-'A',									// drvnum
					CMacXFS::MacDir,								// drvType
					Globals.s_Preferences.m_drvAlias['C'-'A'],				// alias
...
{
	DebugInfo("CMagiC::UpdateAtariDoubleBuffer() --- PPCVideoAddr =0x%08x", m_pFgBuffer);
	PPCVideoAddr = m_pFgBuffer;

	if	(m_pBgBuffer)
	{
		DebugInfo("CMagiC::UpdateAtariDoubleBuffer() --- PPCVideo2Addr =0x%08x", m_pBgBuffer);
		PPCVideo2Addr = m_pBgBuffer;
	}
	else
	{
		DebugInfo("CMagiC::UpdateAtariDoubleBuffer() --- PPCVideo2Addr =0x%08x", m_pFgBuffer);
		PPCVideo2Addr = m_pFgBuffer;
	}
}
...
void CMagiC::ChangeXFSDrive(short drvNr)
{
	...
	{
		NewType = (Globals.s_Preferences.m_drvAlias[drvNr] == nil) ? CMacXFS::NoMacXFS : CMacXFS::MacDir;

		m_MacXFS.SetXFSDrive (
					drvNr,				// Laufwerknummer
					NewType,			// Laufwerktyp: Mac-Verzeichnis oder nichts
					Globals.s_Preferences.m_drvAlias[drvNr],
					(Globals.s_Preferences.m_drvFlags[drvNr] & 2) ? false : true,	// lange Dateinamen
					(Globals.s_Preferences.m_drvFlags[drvNr] & 1) ? true : false,	// umgekehrte Verzeichnis-Reihenfolge (Problem bei OS X 10.2!)
					m_RAM68k);
		}
...
int CMagiC::CreateThread( void )
{
	...
	errl = MPCreateQueue(&m_EmuNotifQID);
	if	(errl)
		return(errl);
	...
	errl = MPCreateEvent(&m_EventId);
	if	(errl)
		return(errl);

	// Event für Interrupts erstellen (idle loop aufwecken)

	errl = MPCreateEvent(&m_InterruptEventsId);
	if	(errl)
		return(errl);

	// CriticalRegion für Tastaturpuffer und Bildschirmpufferadressen erstellen

	errl = MPCreateCriticalRegion(&m_KbCriticalRegionId);
	if	(errl)
		return(errl);

	errl = MPCreateCriticalRegion(&m_AECriticalRegionId);
	if	(errl)
		return(errl);

	errl = MPCreateCriticalRegion(&m_ScrCriticalRegionId);
	if	(errl)
		return(errl);
...
//		CDebug::DebugInfo("CMagiC::EmuThread() --- %d 68k-Zyklen", CyclesExecuted);
		...
		{
			MPEnterCriticalRegion(m_ScrCriticalRegionId, kDurationForever);
			UpdateAtariDoubleBuffer();
			m_bScreenBufferChanged = false;
			MPExitCriticalRegion(m_ScrCriticalRegionId);
		}
...
#endif
			MPEnterCriticalRegion(m_KbCriticalRegionId, kDurationForever);
...
*/
			MPExitCriticalRegion(m_KbCriticalRegionId);
#ifdef _DEBUG_KB_CRITICAL_REGION
...
void CMagiC::SendAtariFile(const char *pBuf)
{
...
#endif
	MPEnterCriticalRegion(m_AECriticalRegionId, kDurationForever);
	...
	MPExitCriticalRegion(m_AECriticalRegionId);
}
...
//	CDebug::DebugInfo("CMagiC::SendKeyboard() --- message == %08x, KeyUp == %d", message, (int) KeyUp);
	...
	(void) MPSetEvent(			// aufwecken, wenn in "idle task"
			m_InterruptEventsId,
			EMU_INTPENDING_KBMOUSE);
...
int CMagiC::SendMousePosition(Point where)
{
	...
	//	where.v -= m_pMagiCScreen->m_AtariScreenYOffset;
	if	(where.v < 0)
		where.v = 0;
...
//#pragma unused(AdrOffset68k)
	...
	PtAtariMousePos.h = (short) ((m_pMagiCScreen->m_AtariScreenRect00.right - m_pMagiCScreen->m_AtariScreenRect00.left) >> 1);
	PtAtariMousePos.v = (short) ((m_pMagiCScreen->m_AtariScreenRect00.bottom - m_pMagiCScreen->m_AtariScreenRect00.top) >> 1);
...
UINT32 CMagiC::AtariVsetRGB(UINT32 params, unsigned char *AdrOffset68k)
{
	...
	UInt32 *pValues = (UInt32 *) (AdrOffset68k + CFSwapInt32BigToHost(theVsetRGBParm->pValues));
	DebugInfo("CMagiC::AtariVsetRGB(index=%d, cnt=%d)", CFSwapInt16BigToHost(theVsetRGBParm->index), CFSwapInt16BigToHost(theVsetRGBParm->cnt));

	// durchlaufe alle zu ändernden Farben
	pColourTable = pTheMagiC->m_pMagiCScreen->m_pColourTable;
	if	(pColourTable)
	{
		j = min(pTheMagiC->m_pMagiCScreen->m_ColourTableLen, CFSwapInt16BigToHost(theVsetRGBParm->index) + CFSwapInt16BigToHost(theVsetRGBParm->cnt));
		for	(i = theVsetRGBParm->index, pColourTable += CFSwapInt16BigToHost(theVsetRGBParm->index);
			i < j;
			i++, pValues++,pColourTable++)
		{
		//	pColourTable->value = i; 	// ?
			c = (unsigned short) ((CFSwapInt32BigToHost(*pValues) & 0x00ff0000) >> 16);
			rgb.red = (unsigned short) ((c << 8) | c);
			c = (unsigned short) ((CFSwapInt32BigToHost(*pValues) & 0x0000ff00) >> 8);
			rgb.green = (unsigned short) ((c << 8) | c);
			c = (unsigned short) ((CFSwapInt32BigToHost(*pValues) & 0x000000ff) >> 0);
			rgb.blue = (unsigned short) ((c << 8) | c);

			pColourTable->rgb = rgb;
		}
...
UINT32 CMagiC::AtariVgetRGB(UINT32 params, unsigned char *AdrOffset68k)
{
	...
	struct VgetRGBParm
	{
 	...
 	UInt32 *pValues = (UInt32 *) (AdrOffset68k + CFSwapInt32BigToHost(theVgetRGBParm->pValues));
	DebugInfo("CMagiC::AtariVgetRGB(index=%d, cnt=%d)", CFSwapInt16BigToHost(theVgetRGBParm->index), CFSwapInt16BigToHost(theVgetRGBParm->cnt));

	// durchlaufe alle zu ändernden Farben
	pColourTable = pTheMagiC->m_pMagiCScreen->m_pColourTable;
	if	(pColourTable)
	{
		j = min(pTheMagiC->m_pMagiCScreen->m_ColourTableLen, CFSwapInt16BigToHost(theVgetRGBParm->index) + CFSwapInt16BigToHost(theVgetRGBParm->cnt));
		for	(i = CFSwapInt16BigToHost(theVgetRGBParm->index), pColourTable += CFSwapInt16BigToHost(theVgetRGBParm->index);
			i < j;
			i++, pValues++, pColourTable++)
		{
			*pValues = CFSwapInt32HostToBig((unsigned long) (pColourTable->rgb.red >> 8) << 16 +
					(pColourTable->rgb.green >> 8) << 8 +
					(pColourTable->rgb.blue >> 8));
		}
	}
	return(0);
}
...
void *CMagiC::_Remote_AtariSysHalt( void *param )
{
#pragma unused(param)
	// Unter OS X können wir das Ereignis synchron bearbeiten
	SendMessageToMainThread(false, FOUR_CHAR_CODE('halt'));
	return(0);
}
...
// Daten werden getrennt von der Nachricht geliefert
	...
	return((UINT32) MPRemoteCall(_Remote_AtariSysHalt, (void *) AdrOffset68k, kMPOwningProcessRemoteContext));
	return(0);
}
...
void *CMagiC::_Remote_AtariSysErr( void *param )
{
#pragma unused(param)
	// Unter OS X können wir das Ereignis synchron bearbeiten
	SendMessageToMainThread(false, FOUR_CHAR_CODE('excp'));
	return(0);
}
...
// Daten werden getrennt von der Nachricht geliefert
	...
	Send68kExceptionData(
				(UInt16) CFSwapInt32BigToHost(AdrOffset68k[proc_pc /*0x3c4*/]),		// Exception-Nummer
	...
	return((UINT32) MPRemoteCall(_Remote_AtariSysErr, (void *) AdrOffset68k, kMPOwningProcessRemoteContext));
	return(0);
}
...
void *CMagiC::_Remote_AtariError( void *param )
{
#pragma unused(param)
/*
...
[Quit MagiCMacX]
*/
	MyStopAlert(ALRT_NO_VIDEO_DRIVER);
	// Event-Loop des Haupt-Threads beenden
...
UINT32 CMagiC::AtariError(UINT32 params, unsigned char *AdrOffset68k)
{
	...
	return((UINT32) MPRemoteCall(_Remote_AtariError, (void *) errorCode, kMPOwningProcessRemoteContext));
	return(0);
}
